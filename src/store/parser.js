// import { a, b, c, d, e, f } from '/template/acr.js'

// parser glory
export function glory(value) {
...
export function arystan(value) {
	...
	try {
		for (let i = 0; i < arystanAll.length; i++) {
			...
			const a = new Date(el[2].split(el[2][2]).reverse()).toString().split(' ')
			// console.log(el)
			// console.log(el[2][2])
			// console.log(a)
...
export function skyUp(value) {
	try {
		if (value.includes('Changes of your flight')) {
			...
			let date = new Date(info[0][0].split('/').reverse()).toString().split(' ')
			// console.log(info)
			// console.log(date)
	...
	try {
		...
		for (let j = 0; j < s7Filter.length; j++) {
			...
			s7 += `${el[2]} ${date[2]}${date[1].toUpperCase()}${date[3].slice(2)} ${
				el[3]
			}${el[5]} ${el[4].replace(':', '')} ${el[6].replace(':', '')}\n`
		}
		return s7
	} catch (error) {
		return 'Некоректний вибір опції або неповний/некоректний рядок</br>'
		...
		amadeus = value[1]
			...
			.map((e) => {
				...
				return `SR DOCS YY HK1-P-${e[6].slice(-3, -1)}-${
					...
					documentValidity[2]
				}${documentValidity[1].toUpperCase()}${documentValidity[3].slice(-2)}-${
					e[1]
				}-${e[2]}/P`
			})
			.join('</br>')
